...
data class Contact(val name: String, val phoneNumber: Int , val address: String)
...
class MyBinary {
    ...
    private fun insertNode(currentNode: MyNode, newNode: MyNode) {
    ...
    private fun findMinNode(currentNode: MyNode): MyNode {
        if (currentNode.leftChild == null) {
            return currentNode;
        }
    ...
    private fun findParentNodeByChildKey(currentNode: MyNode, key: Int) : MyNode? {
    ...
    fun deleteNode(key: Int) {
        ...
        val parentOfSuccessor = if (nodeSuccessor != null && nodeToDelete !== this.rootNode)  this.findParentNodeByChildKey(this.rootNode!!, nodeSuccessor!!.value.phoneNumber) else null;
        // Remove successor from parent node of successor children
        if (parentOfSuccessor?.leftChild != null) {
           parentOfSuccessor.leftChild = null
        }
        // assign node successor to parent child of node to delete
        ...
        if (nodeSuccessor != nodeToDelete.rightChild) { nodeSuccessor?.rightChild = nodeToDelete.rightChild } // assign nodeSuccessor.rightChild only if findMinNode got a left leaf node.
        if (rootNode === nodeToDelete) { this.rootNode = nodeSuccessor }
    }

    private fun findNodeByKey(currentNode: MyNode, key: Int): MyNode? {
        return when {
            key < currentNode.value.phoneNumber -> {
...
fun main() {
    ...
    myBinary.addNode(Contact("nico", 10, "24 allée gabriel peri"))
    ...
    println(myBinary.searchNode(10))
}
